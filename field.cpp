...
void DrawField(void)
{
	...
	for (int i = 0; i < FIELD_TILE_H; i++)
	{
		...
		{
			// 画面外まで進んだ？
			XMFLOAT3 pos = XMFLOAT3(j * tiles->w + tiles->w / 2 - bg->pos.x,
				i * tiles->h + tiles->h / 2 - bg->pos.y,
				0);
			if ((pos.y < (-tiles->h / 2)) ||			// 自分の大きさを考慮して画面外か判定している
				(pos.y > (bg->h +tiles->h / 2)) ||
				(pos.x < (-tiles->w / 2)) ||
				(pos.x > (bg->w + tiles->w / 2)))
			{
				continue;
			}
